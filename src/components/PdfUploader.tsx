...
export default function PdfUploader({ file, onFileChange }: PdfUploaderProps) {
  ...
  if (rawFile) {
    return (
      <div className="rounded-xl border-2 border-[#E4E1DA] bg-[#F3F1EC] p-4">
        {/* File info */}
        <div className="mb-3 flex items-center gap-3">
          <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-[#294C67]/10 text-xs font-bold text-[#16202B]">
            PDF
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-[#16202B]">{rawFile.name}</p>
          ...
          <button
            onClick={handleReset}
            className="rounded-lg p-1 text-[#5B6470] hover:bg-red-50 hover:text-red-500"
            title="삭제"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        {/* Page range input */}
        <p className="mb-2 text-xs text-[#5B6470]">
          작품 본문이 수록된 페이지 번호를 정확히 입력하세요. 해당 페이지만 추출하여 주석을 인식합니다.
        </p>
        <div className="flex items-center gap-2">
          <label className="flex-shrink-0 text-xs font-medium text-[#16202B]">
            페이지 범위
          </label>
          <input
            type="text"
            ...
            className="min-w-0 flex-1 rounded-lg border border-[#E4E1DA] px-3 py-1.5 text-xs text-[#16202B] placeholder-[#7C8492] outline-none focus:border-[#294C67]"
            onKeyDown={(e) => { if (e.key === "Enter") handleExtractPages(); }}
          />
          <button
            onClick={handleExtractPages}
            disabled={!pageRange.trim() || isExtracting}
            className="flex flex-shrink-0 items-center gap-1 rounded-lg bg-[#294C67] px-3 py-1.5 text-xs font-medium text-white transition-all hover:bg-[#21405A] disabled:opacity-40"
          >
    ...
    <div>
      <div
        ...
        className={`flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 border-dashed px-4 py-6 transition-colors ${
          isDragging
            ? "border-[#294C67] bg-[#F3F1EC]"
            : "border-[#E4E1DA] hover:border-[#294C67]/50 hover:bg-[#F3F1EC]"
        }`}
      >
        <FileUp className="h-6 w-6 text-[#6E7683]" />
        <p className="text-sm text-[#5B6470]">
          교과서 PDF를 드래그하거나 클릭하여 업로드
        </p>
        <p className="text-xs text-[#6E7683]">업로드 후 필요한 페이지만 추출합니다</p>
      </div>
      <input
        ref={inputRef}
        type="file"
