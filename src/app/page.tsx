    ...
    <div className="min-h-screen w-screen overflow-x-hidden bg-gradient-to-br from-white to-[#FFF0E4]">
      ...
      <section className="w-full bg-white px-4 py-20 sm:px-10 lg:px-24">
        ...
        <div className="mx-auto mb-14 max-w-2xl text-center">
          ...
          <h2 className="mb-4 text-3xl font-extrabold tracking-tight text-[#6B3F26]">
        ...
        <div className="mx-auto mb-24 grid max-w-4xl grid-cols-1 gap-5 sm:grid-cols-3">
          {PAIN_POINTS.map((p) => (
            <div
              key={p.stat}
              className="flex flex-col items-center gap-3 rounded-2xl border border-[#EEDDD0] bg-[#FFFBF8] p-7 text-center"
            >
              <Icon name={p.icon} size={28} className="text-[#A07050]" />
              <p className="text-lg font-extrabold text-[#6B3F26]">{p.stat}</p>
        ...
        <div className="mx-auto mb-24 max-w-5xl">
          <div className="mb-10 text-center">
            ...
            <h2 className="text-2xl font-extrabold tracking-tight text-[#6B3F26]">
              수업 준비를 바꾸는 세 가지
            </h2>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {CORE_FEATURES.map((f) => (
              <div
                key={f.title}
                className="flex flex-col gap-4 rounded-2xl border border-[#EEDDD0] bg-[#FFFBF8] p-7"
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#F5E8DC] text-[#A07050]">
                  <Icon name={f.icon} size={24} />
                </span>
                <h3 className="text-base font-bold text-[#6B3F26]">{f.title}</h3>
