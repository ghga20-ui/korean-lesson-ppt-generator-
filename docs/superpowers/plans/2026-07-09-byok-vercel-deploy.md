...
- Delete: `src/app/api/blob-token/route.ts` (구현 중 발견 — 저장소 어디서도 호출하지 않는 죽은 코드이며 `@vercel/blob`을 import하고 `BLOB_READ_WRITE_TOKEN` 환경변수를 요구한다. 남겨두면 "환경변수 0개 배포"가 불가능하다. 사용자 승인 후 삭제 확정)
...
Run: `grep -rn "api/extract\|start-file-upload\|upload-chunk\|upload-pdf\|blob-token\|gemini-server\|vercel/blob\|gemini-file-upload\|waitForFileActiveByUri" src/ test/`
Expected: 삭제 대상 파일 자기 자신 외의 참조 0건. 참조가 남아 있으면 먼저 제거.

- [ ] **Step 2: 파일 삭제 및 의존성 제거**

```bash
git rm src/app/api/extract/route.ts src/app/api/start-file-upload/route.ts src/app/api/upload-chunk/route.ts src/app/api/upload-pdf/route.ts src/app/api/blob-token/route.ts src/lib/gemini-server.ts src/lib/gemini-file-upload.ts test/gemini-file-upload.test.ts
...
이 세 경로는 git에 **커밋된 적이 없다(untracked)** — `git rm`은 `did not match any files` 오류로 실패한다. 평범한 `rm`을 쓴다. `scripts/`에는 `deploy-cloud-run.ps1` 하나뿐이므로 디렉터리째 지운다.

```bash
rm -f docs/cloud-run-cli.md .gcloudignore
rm -rf scripts/
```

- [ ] **Step 3: Commit**

`git add -A`는 저장소의 다른 untracked 잡동사니(`.omx/`, `output/`, `NanumBarunGothicYetHangul/*.ttf`, `docs/superpowers/plans/2026-04-09-*.md`)까지 빨아들인다. **README 하나만** 스테이징한다. (삭제 대상은 untracked였으므로 커밋할 것이 없다.)
