...
- 인라인 전송 상한 15MB (`INLINE_MAX_BYTES = 15 * 1024 * 1024`). 초과분은 File API 업로드(Task 1 스파이크 결과에 따라 유지/제거).
...
**판정 결과: (구현 시 기록) FILE_API_SUPPORTED = ___**
...
`gemini-server.ts`의 로직을 클라이언트용으로 이식한다. 프롬프트·2라운드 검증·타임아웃·에러 매핑은 동작 그대로, 다음만 변경: ① 키/모델을 인자로 받음 ② `Buffer` 대신 브라우저 호환 base64 ③ 크기 기반 전송 분기 ④ 테스트를 위한 `fetchImpl` 주입.

**Files:**
- Create: `src/lib/gemini-client.ts`
- Test: `src/lib/gemini-client.test.ts`
- 참조(복사 원본, 이 태스크에서는 수정하지 않음): `src/lib/gemini-server.ts`, `src/lib/gemini-file-upload.ts`

**Interfaces:**
- Consumes: `uploadPdfToGeminiFile(pdf: File, apiKey: string, fetchImpl?)` (기존 gemini-file-upload.ts — 이미 fetch 주입 지원, 수정 불필요)
- Produces (Task 6이 사용):
  ...
  - `FILE_API_SUPPORTED: boolean` (Task 1 판정값을 상수로 기록)
...
describe("extractFromPdfClient", () => {
  ...
  it("INLINE_MAX_BYTES는 15MB", () => {
    expect(INLINE_MAX_BYTES).toBe(15 * 1024 * 1024);
  });
});
```
...
/** 인라인 base64 전송 상한 (Gemini 요청 한도 20MB에 안전 마진) */
export const INLINE_MAX_BYTES = 15 * 1024 * 1024;

/** Task 1 CORS 스파이크 판정값. false면 15MB 초과 PDF를 거부한다. */
export const FILE_API_SUPPORTED = true; // ← Task 1 결과로 확정
...
export async function extractFromPdfClient(
  ...
  if (pdf.size <= INLINE_MAX_BYTES) {
    ...
    pdfData = { base64: await fileToBase64(pdf) };
  } else if (FILE_API_SUPPORTED) {
    onProgress?.("PDF 업로드 중... (대용량)");
    const fileUri = await uploadPdfToGeminiFile(pdf, apiKey, fetchImpl);
    pdfData = { fileUri };
  } else {
    throw new Error("PDF가 15MB를 초과합니다. 페이지 범위를 더 좁게 추출한 뒤 다시 시도해주세요.");
  }
...
주의: Task 1 판정이 `false`였다면 `FILE_API_SUPPORTED = false`로 설정하고 `uploadPdfToGeminiFile` import는 제거한다(분기 코드에서 미사용이 되므로 lint 오류 방지).

- [ ] **Step 4: 테스트 통과 확인**

Run: `npm test`
Expected: PASS (api-key-storage 7 + gemini-client 7)
...
```js
await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${localStorage.getItem("lit-ppt-gemini-key")}&pageSize=50`)
  .then(r => r.json()).then(d => d.models.map(m => m.name).filter(n => n.includes("flash")));
```

Expected: 목록에 `models/gemini-flash-latest`(또는 동등 별칭)가 존재. 없으면 `src/lib/gemini-models.ts`의 Flash id를 목록에 실재하는 최신 flash 모델로 교체.
...
`npm run dev`에서:
1. Mode C: 본문 붙여넣기 + 교과서 PDF(페이지 추출, ~5MB) → "주석 추출 + 분할" → 주석 달린 슬라이드 생성 확인. 개발자 도구 Network 탭에서 요청이 `generativelanguage.googleapis.com`으로만 나가고 `/api/extract` 호출이 **없는지** 확인
2. Mode A: PDF만으로 텍스트+주석 추출 확인
3. PPTX 생성까지 완주 (`.pptx` 다운로드 및 PowerPoint에서 열기)
4. (FILE_API_SUPPORTED = true인 경우) 15MB 초과 PDF로 File API 경로 확인 / (false인 경우) 15MB 초과 시 안내 메시지 확인
...
- Modify: `src/lib/gemini-file-upload.ts` (`waitForFileActiveByUri` — gemini-server 전용이었으므로 삭제; `FILE_API_SUPPORTED = false`로 확정된 경우 파일 전체 삭제)
...
```bash
git rm src/app/api/extract/route.ts src/app/api/start-file-upload/route.ts src/app/api/upload-chunk/route.ts src/app/api/upload-pdf/route.ts src/lib/gemini-server.ts
npm uninstall @vercel/blob
```

`gemini-file-upload.ts`에서 `waitForFileActiveByUri` 함수(및 주석) 삭제. `FILE_API_SUPPORTED = false`인 경우 `git rm src/lib/gemini-file-upload.ts`.
...
| 구분 | 항목 |
|------|------|
| 삭제 | `/api/extract`, `/api/start-file-upload`, `/api/upload-chunk`, `/api/upload-pdf`, `gemini-server.ts`, `@vercel/blob`, `GEMINI_API_KEY`, Cloud Run 문서/스크립트 |
| 신규 | `gemini-client.ts`, `gemini-models.ts`, `api-key-storage.ts`, `ApiKeySettings.tsx`, vitest |
| 유지 | `/api/generate-pptx`, `/api/generate-html`, pptx 생성 계열, `annotation-matcher`, `slide-splitter`, `gemini-file-upload.ts`(FILE_API_SUPPORTED = true인 경우) |
